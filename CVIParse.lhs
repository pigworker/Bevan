...
> spcu :: CVIP ()
> spcu = () <$ many (eat (\ c -> isSpace c || elem c "#_"))
...
> tyShow :: Type String -> String
> tyShow (V x) = x
> tyShow B = "B"
> tyShow (T ts) = "(" ++ intercalate " " (map tyShow ts) ++ ")"
> tyShow (ss :-> lts) = "[" ++ intercalate " " (map tyShow ss) ++ " -> "
>   ++ intercalate " " (map (\ (l, t) -> replicate l '+' ++ tyShow t) lts) ++ "]"
...
>     traverse (trawerse (traverse f)) its where
>     trawerse f (i, t) = (,) i <$> f t
...
> pDeclIn = (,) <$ spcu <*> pName <*> many (spcu *> pType) <* spcu <* eol
...
> cviDefn (f, is) iI lz (s : ss) = case span isSpace s of
>   (_, '=' : _) ->  case cvip pInterface (0, s) of
...
>   _ -> case cvip pTiles (0, s) of
>     Nothing -> Left ("line of components for " ++ f, s)
>     Just (l, _) -> cviDefn (f, is) iI (l : lz) ss
